...
/**
 * Home object for domain model class Quote.
 * @see it.crem.db.conf.Quote
 * @author Hibernate Tools
 */
public class QuoteHome {

	private static final Log log = LogFactory.getLog(QuoteHome.class);
	...
	public void persist(Quote transientInstance) {
		log.debug("persisting Quote instance");
		try {
	...
	public Quote merge(Quote detachedInstance) {
		...
		try {
			Quote result = (Quote) sessionFactory.getCurrentSession().merge(
					detachedInstance);
	...
	public Quote findById(int id) {
		...
		try {
			Quote instance = (Quote) sessionFactory.getCurrentSession().get(
					"it.crem.db.conf.Quote", id);
			if (instance == null) {
	...
	public List findByExample(Quote instance) {
		...
		try {
			List results = sessionFactory.getCurrentSession()
					.createCriteria("it.crem.db.conf.Quote")
