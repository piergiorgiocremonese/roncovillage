...
function ckSubFormRagazzo(){
	...
	if (addrv=''){
		alert('manca Indirizzo: campo obbligatorio');
		ok=false;
	}
	if (capv=''){
		alert('manca CAP: campo obbligatorio');
		ok=false;
	}
	if (cittav=''){
		alert('manca Citta di residenza: campo obbligatorio');
		ok=false;
	}
	if (nrv == ''){
		alert('manca nome Ragazzo: campo obbligatorio');
		ok=false;
	}
	if (cnrv == ''){
		alert('manca cognome Ragazzo: campo obbligatorio');
		ok=false;
	}
	if (dnv == ''){
		alert('manca data nascita Ragazzo: campo obbligatorio');
		ok=false;
	}
	if (cnv == ''){
		alert('manca citta di nascita Ragazzo: campo obbligatorio');
		ok=false;
	}
	return ok;
...
function ckSubFormGenitore(){
	...
	if ((cfpv=='')&&(cfmv == '')){
		alert('manca codice fiscale per almeno un Genitore: inserire quello cui si intesta il pagamento');
		ok= false;
	}
...
function ckSubForm(div){
	var ok = true;
	if (div == 'dati-ragazzo')
		ok = ckSubFormRagazzo();
	else if (div == 'dati-genitore')
			ok = ckSubFormGenitore();
	else
		ok=true;
	return ok;			
...
function avanti(divprev,divnext){
	...
	if (ok){
		...
		alert('errore nella form controllare i campi mancanti');
	}
	//pe.style.left="30px"
	
}

function indietro(divprev,divnext){
...
<div id="blockcnt" style="color:white">
...
<tr><td colspan="1" style="color:white" ><center><b>Scheda di iscrizione</b></center></td></tr>
...
<tr id="button-row-1">
<td colspan="2">
<input type="button" value="avanti" onclick="javascript:avanti('dati-ragazzo','dati-genitori')" />
</td>
...
<div id="calendario" style="width:400px;height:200px;background-color=white"></div>
</div>
...
<tr id="button-row-2">
<td colspan="1"><input type="button" value="avanti" onclick="javascript:avanti('dati-genitori','dati-generali')" />
</td>
<td colspan="1">
<input type="button" value="indietro" onclick="javascript:indietro('dati-genitori','dati-ragazzo')" />
</td>
</tr>
